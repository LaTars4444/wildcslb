...
export default function KickAuthPage() {
  ...
  const redirectUri =
    typeof window !== "undefined"
      ? `${window.location.origin}/auth/kick/callback`
      : "";

  const authUrl = clientId
    ? `https://kick.com/oauth/authorize?client_id=${encodeURIComponent(
        clientId
  ...
  return (
    <main className="min-h-screen px-6 py-20">
      <div className="mx-auto max-w-3xl rounded-[2rem] border border-[var(--border-color)] bg-[var(--surface-color)]/90 p-10 shadow-[0_40px_120px_-40px_rgba(0,0,0,0.6)] backdrop-blur-xl">
        <p className="text-sm uppercase tracking-[0.22em] text-[var(--accent-color)]">Kick Login</p>
        <h1 className="mt-2 text-3xl font-black text-[var(--text-primary)]">Connect your Kick account</h1>
        <p className="mt-4 text-[var(--text-secondary)]">
          Login with Kick to link watch time tracking and sync your WildCS rewards.
        </p>

        <div className="mt-10 grid gap-6 sm:grid-cols-2">
          <div className="rounded-[1.75rem] border border-[var(--border-color)] bg-[var(--elevated-color)]/70 p-6">
            <p className="font-semibold uppercase tracking-[0.18em] text-[var(--text-secondary)]">Kick connection</p>
            <p className="mt-3 text-[var(--text-primary)]">
              Use this route to initiate Kick OAuth. Once connected, watch hours can be tracked from your Kick session.
            </p>
          </div>
          <div className="rounded-[1.75rem] border border-[var(--border-color)] bg-[var(--surface-color)]/85 p-6">
            ...
            <p className="mt-3 text-[var(--text-secondary)]">
              Set <code className="rounded bg-black/20 px-1 py-0.5">NEXT_PUBLIC_KICK_CLIENT_ID</code> and optionally <code className="rounded bg-black/20 px-1 py-0.5">NEXT_PUBLIC_KICK_REDIRECT_URI</code>.
            </p>
          </div>
        </div>
