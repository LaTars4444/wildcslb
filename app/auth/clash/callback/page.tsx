...
export default function ClashCallbackPage() {
  const [status] = useState(() => {
    if (typeof window === "undefined") {
      return "Completing Clash login...";
    }
    ...
    if (!code) {
      return "Clash login did not return an auth code. Please try again.";
    }

    window.localStorage.setItem("wildcs_clash_auth", "true");
    return "Clash login completed. Your wager tracking connection is now stored locally.";
  });
  ...
  return (
    <main className="min-h-screen px-6 py-20">
      <div className="mx-auto max-w-3xl rounded-[2rem] border border-[var(--border-color)] bg-[var(--surface-color)]/90 p-10 shadow-[0_40px_120px_-40px_rgba(0,0,0,0.6)] backdrop-blur-xl">
        <p className="text-sm uppercase tracking-[0.22em] text-[var(--accent-color)]">Clash callback</p>
        <h1 className="mt-2 text-3xl font-black text-[var(--text-primary)]">Clash authorization result</h1>
