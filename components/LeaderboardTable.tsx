...
const MIN_ROWS = 10; // Minimum rows to display
...
export default function LeaderboardTable({
  ...
  const emptyRowsNeeded = Math.max(0, MIN_ROWS - userCount);
  const showPlaceholders = emptyRowsNeeded > 0;
        ...
        <tbody>
          {hasUsers &&
            users.map((user, index) => (
              <tr
                key={user.id}
                className="border-t border-[var(--border-color)] hover:bg-[rgba(255,255,255,0.05)]"
              >
                ...
                <td className="px-4 py-4">
                  <Link
                    href={`/claim?user=${encodeURIComponent(user.username)}`}
          ...
          {showPlaceholders &&
            placeholders.slice(0, emptyRowsNeeded).map((item, index) => (
              <tr
                key={`placeholder-${item.id}-${index}`}
                className="border-t border-[var(--border-color)] hover:bg-[rgba(255,255,255,0.04)]"
              >
                <td className="px-4 py-4 text-sm text-[var(--text-secondary)]">
                  {userCount + index + 1}
                </td>
                <td className="flex items-center gap-3 px-4 py-4">
                  <div className="relative h-10 w-10 overflow-hidden rounded-3xl border border-[var(--border-color)] bg-[var(--bg-color)]">
                    <Image
                      src={item.logo}
                      alt={item.name}
                      fill
                ...
                <td className="px-4 py-4">
                  <button
                    disabled
                    className="rounded-full border border-[var(--border-color)] bg-[var(--surface-color)]/80 px-3 py-1 text-sm text-[var(--text-secondary)]"
                  >
                    {item.status}
                  </button>
                </td>
